<template>
  <div class="row">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      ...
      <div class="card">
        <div class="card-header">Reporte de voluntarios inscritos por Sumate
        </div>
        <div class="card-body">
          <!--Calendar-->
...
export default {
    ...
    methods: {
      ...
      async getVolunteers(){
        this.isLoading = true;
        var form = {};
        this.region != ''  ? form['idRegion'] = this.region : null
        if(this.dateRange.start && this.dateRange.end){
          form['startDate'] = this.dateRange.start;
          form['endDate'] = this.dateRange.end;
        }
        if(form != {}){
          const response =  await consultServices('getVolunteerRegisterBySumate','POST', form);
      ...
      async exporteVolunteers () {
         ...
         if(this.volunteers.length > 1){
           ...
           data = _.concat(data, data_body);
             ...
             var auxArrayblue = ['A1', 'B1', 'C1', 'D1', 'E1', 'F1', 'G1', 'H1'];
             ...
             _.map(ws, (cell, key) => {
               ...
               if(key != "!ref"){
                 _.map(auxArrayblue, (blue) => {
                   if(key == blue){
                     wb.Sheets.Reporte_Sumate[key]['s']=jsonBlue;
                     indxColor = 0
                     flagHeader = true;
                   }
                 })
                 if(flagHeader == false){
                   if(indxColor == 0){
                     wb.Sheets.Reporte_Sumate[key]['s']=jsonDataGris;
                     contRow++;
                     ...
                     wb.Sheets.Reporte_Sumate[key]['s']=jsonData;
                     contRow++;
                     if(tempFormat.length <= contRow){
                       contRow = 1 ;
                       indxColor = 0;

  ...
  <div class="row">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="card">
        <div class="card-header">
          <h2>
            <font style="vertical-align: inherit;">Vista Previa</font>
          </h2>
          <div class="nav navbar-right panel_toolbox">
            ...
            <input
              ...
              type="file"
              id="file-excel"
              @change="changeFileExcel"
              style="display: none;"
            />
            <button
              v-if="(isOver==true && modeLoading==true) || (isOver==false && modeLoading==false)"
              type="button"
        ...
        <div class="card-body">
          <TableMaf :title="tableTableFlag" :header="colum" :data="excelData" ref="tableMaf"></TableMaf>
          <button
            v-if="!modeLoading && excelData.length>0"
            type="button"
            @click="startUpload()"
            data-toggle="modal"
            data-target=".bs-example-modal-lg"
          ...
          <div v-if="modeLoading">
            ...
            <div :style="styleMessageBar.wrapper">
              <span
                :style="styleMessageBar.text"
              >{{dataSent}} datos enviados, se han encontrado {{errorsMessages.length}} errores.</span>
            </div>
            <div v-if="!isOver">
              <button
                v-if="isUploading"
                type="button"
                @click="showStopUpload()"
                data-toggle="modal"
                data-target=".bs-example-modal-lg"
                class="btn btn-danger btn-xs"
              >{{labelStopData}}</button>
              <button
                v-if="!isUploading"
                type="button"
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                      <input
                        ...
                        id="name"
                        class="form-control"
                        :value="dataSent-errorsMessages.length"
                      />
                    </div>
                  </div>
                </div>
            ...
            <div
              v-for="error in errorsMessages"
              class="alert alert-danger alert-dismissible fade in show"
              role="alert"
            >
      ...
      <div class="modal-dialog modal-cat">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <h3>
              <center>¿Seguro que quiere terminar?</center>
            </h3>
          </div>
    ...
    <div class="modal fade bs-example-modal-success" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog modal-cat">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">La carga a finalizado {{ errorsMessages.length > 0? 'con error': ''}}</h4>
            <button class="close" type="button" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">×</span>
            </button>
          </div>
            ...
            <div v-if="errorsMessages.length > 0">
               ...
               <br>
                ...
                <center>
                  ...
                  <span>{{dataSent-errorsMessages.length}}</span>
                </center>
              </h3>
               <br>
              <h3>
                <center>
                  Enviados con errores:
                  <span>{{errorsMessages.length}}</span>
                </center>
              </h3>
            </div>
            <h3 v-else>
              <center>Se han subido exitosamente</center>
            </h3>
          </div>
          <div class="modal-footer">
            <center>
...
export default {
  ...
  methods: {
    ...
    async startUpload(isContinue = false) {
      ...
      for (
        ;
        this.positionUploading < this.excelData.length && this.isUploading;

      ) {
        //PORCION DE DATA QUE SE ENVIA
        var _data = _.slice(
          this.excelData,
          this.positionUploading * this.slice,
          (this.positionUploading + 1) * this.slice > this.excelData.length
            ? this.excelData.length
            : (this.positionUploading + 1) * this.slice
        );
    ...
    changeFileExcel() {
      ...
      var files = this.$refs.myFilesExcel.files,
        f = files[0];
      if (files[0]) {
        this.isLoading = true;
